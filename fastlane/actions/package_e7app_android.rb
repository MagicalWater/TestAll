module Fastlane
  module Actions
    class PackageE7appAndroidAction < Action
      ...
      def self.run(params)
        ...
        if isDebug
          UI.important "Android E7-app, 打包模式: debug"
        else
          UI.important "Android E7-app, 打包模式: release, 為求安全性, 暫時不開放 release"
          return
        end
      ...
      def self.update_apk(is_debug, output_path)
        ...
        if is_debug
          ...
          targetPath = "/var/www/html/apk/app_debug.apk"
        else
          apkPath = "#{output_path}/release.apk"
          targetPath = "/var/www/html/apk/app_release.apk"
        end
      ...
      def self.modify_version_file(is_debug)
        ...
        androidHash = versionParseHash["e7App"]["android"]
        if is_debug
        ...
        versionParseHash["e7App"]["android"] = androidHash
