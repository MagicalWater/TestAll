module Fastlane
  module Actions
    class ProjE7PackageAndroidAction < Action
      ...
      def self.run(params)
        ...
        if increaseVersion == "3"
          ...
          $isVersionKeep = true
          versionHash = ProjE7VersionAction.get_new_version("android", "app", $isDebug, $isRelease, $isProduct)
          specialVersion = "#{versionHash["name"]}+#{versionHash["code"]}"
          puts "版本保持不變 - #{specialVersion}"
        else
          UI.important "自動增加版本號"
          $isVersionIncrease = true
          versionHash = ProjE7VersionAction.get_next_version("android", "app", $isDebug, $isRelease, $isProduct)
          specialVersion = "#{versionHash["name"]}+#{versionHash["code"]}"
        ...
        # 上傳 apk
        ProjE7UpdateAction.update_package("android", "app", apkPath)

        # 修改版本號碼
        ProjE7VersionAction.modify_version("android", "app", $isDebug, $isRelease, $isProduct)
