module Fastlane
  module Actions
    class ProjE7BackendPackageIosAction < Action
      ...
      def self.run(params)
        ...
        ProjE7BackendPackageAndroidAction.check_is_e7_backend_project()
        ...
        if increaseVersion == "3"
          ...
          $isVersionKeep = true
          versionHash = ProjE7VersionAction.get_new_version("ios", "backend", $isDebug, $isRelease, $isProduct)
          specialVersion = "#{versionHash["name"]}+#{versionHash["code"]}"
          puts "版本保持不變 - #{specialVersion}"
        else
          UI.important "自動增加版本號"
          $isVersionIncrease = true
          versionHash = ProjE7VersionAction.get_next_version("ios", "backend", $isDebug, $isRelease, $isProduct)
          specialVersion = "#{versionHash["name"]}+#{versionHash["code"]}"
        ...
        # 將 ipa 上傳至伺服器
        ProjE7UpdateAction.update_package("ios", "backend", ipaPath)

        # 修改版本號碼
        ProjE7VersionAction.modify_version("ios", "backend", $isDebug, $isRelease, $isProduct)
