module Fastlane
  module Actions
    class PackageE7backendIosAction < Action
      ...
      def self.run(params)
        ...
        if isDebug
          UI.important "IOS E7-超級代理, 打包模式: debug"
        else
          UI.important "IOS E7-超級代理, 打包模式: release, 為求安全性, 暫時不開放 release"
          return
        end
      ...
      def self.update_ipa(is_debug, output_path)
        ...
        if is_debug
          ...
          targetPath = "/var/www/html/ipa/backend_debug.ipa"
        else
          ipaPath = "#{output_path}/release.ipa"
          targetPath = "/var/www/html/ipa/backend_release.ipa"
        end
      ...
      def self.modify_version_file(is_debug)
        ...
        iosHash = versionParseHash["e7Backend"]["ios"]
        if is_debug
        ...
        versionParseHash["e7Backend"]["ios"] = iosHash
